...
def update(time_diff, model: piezo_film, ax):
    ...
    force_a = 2
    force_b = 2
    model.set_force(lambda t: force_a if (t < 0.001 or (t > diff and t < diff + 0.001)) else 0)
    r = model.voltage(method='DOP853')
...
ani.save('Cantilever_mass_fcn_python_two.gif', writer=writer)
